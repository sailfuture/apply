        ...
        {enrolled.length === 0 ? (
          ...
          <div className="space-y-4">
            {enrolled.map(({ app, student }) => {
              ...
              return (
                // One card per student. Header carries the avatar /
                // name / status icon; body stacks the schedule CTA
                // and the confirmation checkbox separated by a thin
                // gray rule. Keeping the two affordances inside the
                // same card reads as one acknowledgement step
                // anchored to the student.
                <Card key={student.id} className="overflow-hidden gap-0 py-0">
                  ...
                  <CardContent className="py-5 bg-white dark:bg-background space-y-4">
                    ...
                    {/* Thin horizontal rule between the schedule CTA
                        and the confirmation checkbox — visually
                        separates the two steps without needing a
                        full second card. */}
