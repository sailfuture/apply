 ...
 * admin actually exports (mirrors how the family-PDF export lazy-loads
 * jsPDF). Rows become spreadsheet rows; the chosen `columns` become the
 * header + cell projection, in the order given.
 */
export async function exportEnrolledXlsx({
  rows,
  columns,
  filename,
}: {
  rows: EnrolledExportRow[];
