...
/** Lowercase a name into an email-safe token: strip diacritics
 *  (NFD then drop combining marks), drop apostrophes/spaces/hyphens
 *  and anything else non-alphanumeric ("O'Brien-Smith" → "obriensmith"). */
...
export function generateSchoolPassword(
  ...
  const firstInitial = emailToken(firstName).charAt(0).toUpperCase();
  const lastInitial = emailToken(lastName).charAt(0).toUpperCase();
  const startYear = schoolYearStartYear(yearName);
